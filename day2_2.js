const input = `104 240 147 246 123 175 372 71 116 230 260 118 202 270 277 292
...
const main = () => {
  ...
  const solution = rows.reduce((checksum, row) => {
    const rowNumbers = row.split(' ').map(Number);
    let num1 = 0;
    let num2 = 0;
    rowNumbers.forEach(num => {
      const dividable = rowNumbers.filter(x => x !== num && num % x === 0);
      if (dividable.length) {
        num1 = num;
        num2 = dividable[0];
      }
